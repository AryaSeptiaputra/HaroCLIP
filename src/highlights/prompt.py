...
def build_system_prompt(campaign_context: str | None = None) -> str:
    ...
    return (
        ...
        "hook rules above, not a replacement for them. A clip must still be a genuine "
        "hook (curiosity gap, surprising claim, emotional peak, concrete insight, or "
        "controversial opinion) on its own merits — campaign relevance only breaks "
        "ties between candidates that already clear that bar, and never substitutes "
        "for it.\n\n"
        "This matters most when the campaign context below is heavy on product specs, "
        "mandatory slogans, required timestamps, or a list of keywords/hashtags to "
        "include: that content describes what the campaign wants covered, not a green "
        "light to select a clip just because it covers it. Apply this test to every "
        "campaign-relevant candidate before selecting it: if you stripped out every "
        "campaign-specific word, would the clip still be a hook under the base rules "
        "above? If yes, the product angle is riding along inside a moment that would "
        "already stop a scrolling viewer — select it freely. If no, the clip's only "
        "reason to exist is that it covers required content — reject it, even if the "
        "brief calls that moment mandatory, and even if that means a mandatory item "
        "goes uncovered.\n\n"
        "Concretely reject or deprioritize clips that are fundamentally: reciting a "
        "product's features, specs, or benefits with no narrative tension; reading out "
        "a list of required slogans, hashtags, or keywords; or walking through "
        "application/usage instructions step by step. These may be informative to a "
        "buyer, but they are not something a scrolling viewer stops for, no matter how "
        "campaign-relevant they are.\n\n"
        "Instead, actively look for moments where the campaign's product or message "
        "is carried inside a genuine hook — for example: a funny or surprising "
        "exchange that naturally mentions the product mid-joke; a relatable emotional "
        "moment that happens to involve using it; a concrete insight or strongly-"
        "stated opinion that uses the product as its example. In these, the hook (the "
        "humor, surprise, emotion, insight, or opinion) stands on its own — the "
        "campaign angle rides along inside it, rather than being the reason for the "
        "clip.\n\n"
